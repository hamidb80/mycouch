...
proc newAsyncCouchDBClient*(host: string = "http://localhost",
    port = 5984): AsyncCouchDBClient =
  ## creates new couchdb client - it is used for APIs
...
proc schedulerJobs*(self: CC or AsyncCC; limit, skip = 0): Future[
    JsonNode] {.captureDefaults, multisync.} =
  ...
  let req = await self.hc.get(fmt"{self.baseUrl}/_scheduler/jobs?" &
      encodeQuery(queryParams))

  castError req
  return (await req.body).parseJson

proc schedulerDocs*(self: CC or AsyncCC; replicatorDB, doc_id = "", limit,
    skip = 0, ): Future[JsonNode] {.captureDefaults, multisync.} =
  ...
  let 
    ...
    req = await self.hc.get fmt"{self.baseUrl}/_scheduler/docs{postfix}"
...
proc changeReshardState*(self: CC or AsyncCC; state: ReshardStates,
    state_reason = "") {.captureDefaults, multisync.} =
...
proc changeReshardJobState*(self: CC or AsyncCC; jobId, state: string,
    state_reason = "") {.captureDefaults, multisync.} =
  ## https://docs.couchdb.org/en/latest/api/server/common.html#put--_reshard-state

  let req = await self.hc.put(fmt"{self.baseUrl}/_reshard/jobs/{jobId}/state",
    $createNadd(
    %* {"state": state},
    [state_reason],
    defaults
...
proc createDB*(self: CC or AsyncCC, db; q, n = -1,
    partitioned = false) {.captureDefaults, multisync.} =
  ## https://docs.couchdb.org/en/latest/api/database/common.html#put--db
  let req = await self.hc.put(fmt"{self.baseUrl}/{db}?" &
    encodeQuery createNadd(
...
proc createDoc*(self: CC or AsyncCC, db; doc: JsonNode, batch = BVNon): Future[
    JsonNode] {.captureDefaults, multisync.} =
...
proc bulkGet*(self: CC or AsyncCC, db; docs: JsonNode, revs = false): Future[
    JsonNode] {.captureDefaults, multisync.} =
...
proc bulkDocs*(self: CC or AsyncCC, db; docs: JsonNode,
    new_edits = true): Future[JsonNode] {.captureDefaults, multisync.} =
...
proc getLocalDocs*(self: CC or AsyncCC, db;
  ...
  let req = await self.hc.post(fmt"{self.baseUrl}/{db}/_local_docs/", $
      createNadd( %* {}, [
    conflicts,
    descending,
    startkey,
...
proc deleteDoc*(self: CC or AsyncCC, db, docid; rev: string, batch = BVNon,
    new_edits = false) {.captureDefaults, multisync.} =
  ...
  let req = await self.hc.delete(fmt"{self.baseUrl}/{db}/{docid}?" &
      encodeQuery(queryParams))
...
proc deleteDocAtt*(self: CC or AsyncCC, db, docid, attname; rev: string,
    batch = BVNon): Future[JsonNode] {.captureDefaults, multisync.} =
  ...
  let req = await self.hc.delete(fmt"{self.baseUrl}/{db}/{docid}/{attname}?" &
      encodeQuery(queryParams))
...
proc createOrUpdateDesignDoc*(self: CC or AsyncCC, db, ddoc; rev = "",
    language = "javascript",
  filters = newJObject(),
  options = newJObject(),
  updates = newJObject(),
  ...
  let req = await self.hc.put(fmt"{self.baseUrl}/{db}/_design/{ddoc}",
    $createNadd(query,
    [
      filters,
      options,
...
proc allDocs*(self: CC or AsyncCC, db; queryObj: JsonNode): Future[
    JsonNode] {.captureDefaults, multisync.} =
